...
import {loanFields,getCompanySettings,transformLoan,generateId,getFormattedDate}  from "./helper.js";
...
const insertRecord = async (res, table, fields, values) => {
  try {
    ...
    sendResponse(res, true, "Record inserted successfully", { id: result.insertId });
  } catch (err) {
...
app.post(
  "/api/upload-bank-response",
  memoryUpload.single("file"),
  (req, res) => uploadBankResponse(db, req, res)
);
// ─────────────────────────────────────────────
// 🔐 AUTH — POST /api/login
...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    // Strip the hashed password before returning user data
    const { Password, ...userWithoutPassword } = user;

    // TODO: Consider issuing a signed JWT here for stateless auth
    // const token = jwt.sign({ userId: user.Id, email: user.Email }, process.env.JWT_SECRET, { expiresIn: '1h' });

    return sendResponse(res, true, "Login successful", { user: userWithoutPassword });
  } catch (err) {
...
app.get("/api/get/:entityName", async (req, res) => {
  ...
  try {
    ...
    for (const [fk, { table, refColumn, columns }] of Object.entries(relations)) {
    ...
    const params = [];
    const validFilters = Object.keys(filters).filter((k) => tableColumns.includes(k));
...
app.delete("/api/delete/:entity/:id", async (req, res) => {
  ...
  try {
    const [result] = await db.query("DELETE FROM ?? WHERE Id = ?", [entity, id]);
...
app.post("/api/:entity/insert", (req, res) => {
  ...
  insertRecord(res, entity, fields, fields.map((f) => req.body[f]));
});
...
app.put("/api/:entity/update", (req, res) => {
  ...
  updateRecord(res, entity, fields, fields.map((f) => req.body[f]), req.body["Id"]);
});
...
app.post("/api/child/loanitems/insert", async (req, res) => {
  ...
  if (!Array.isArray(records) || records.length === 0) {
    return sendResponse(res, false, "Invalid data format: expected a non-empty array");
  }
...
// ─────────────────────────────────────────────
app.post("/api/upload/documentversion/insert", upload.single("file"), async (req, res) => {
  const {
    Id, LoanName, ParentDocument, DocumentName, Name,
...
app.post('/api/emi-response', memoryUpload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const data = xlsx.utils.sheet_to_json(sheet, { range: 5, defval: "", raw: false });
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const debitStatus = (row['Debit Status'] || "").toString().trim().toUpperCase();
      ...
      if (debitStatus === 'SUCCESS') {
        ...
        if (!reason) {
          skipped++;
          rowErrors.push(`Row ${excelRowNum}: Reason is required for REJECTED status`);
        } else {
    ...
    if (validRows.length === 0) {
      return res.json({ success: true, summary: { processed: 0, skipped, errors: rowErrors } });
    }
      ...
      for (let i = 0; i < validRows.length; i += CHUNK_SIZE) {
        ...
        if (rejectedInChunk.length > 0) {
          const insertSql = `
                        ...
                        VALUES ${rejectedInChunk.map(() => '(?, ?, ?, ?, ?, ?, ?, ?)').join(', ')}
                    `;
...
app.post("/api/cron/email-zip/:sheetName", async (req, res) => {
  ...
  try {
    ...
    const PENDING = ["New", "Rejected - Ready to send", "Modified - Ready to send"];
    ...
    const recipientEmail = process.env.BANK_EMAIL_RECIPIENT || "rjneniya1415@gmail.com";
...
app.post("/api/cron/email-emi", async (req, res) => {
  ...
  try {
    ...
    await connection.commit();
    res.json({ success: true, message: `Emailed ${emiIds.length} EMIs successfully.` });
